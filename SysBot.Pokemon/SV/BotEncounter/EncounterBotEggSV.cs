...
public class EncounterBotEggSV(PokeBotState cfg, PokeTradeHub<PK9> hub) : EncounterBotSV(cfg, hub)
{
    ...
    protected override async Task EncounterLoop(SAV9SV sav, CancellationToken token)
    {
        ...
        await SetStick(Base.SwitchStick.LEFT, 10000, 0, 0_500, token).ConfigureAwait(false);
        await SetStick(Base.SwitchStick.LEFT, 10000, 0, 1_000, token).ConfigureAwait(false);
        await SetStick(Base.SwitchStick.LEFT, -5000, 0, 0_500, token).ConfigureAwait(false);
    ...
    private async Task OpenPicnic(CancellationToken token)
    {
        ...
        if (await IsOnOverworld(offset, token).ConfigureAwait(false))
